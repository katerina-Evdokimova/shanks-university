...
private:
	...
	T transform_procedure(const int n, const int order) const;
};
...
template <typename T>
T shanks_transform<T>::transform_procedure(const int n, const int order) const
{
...
T shanks_transform<T>::transform(const int n, const int order) const
{
	...
	{
		return transform_procedure(n, order);
	}
